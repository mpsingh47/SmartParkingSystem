...
class main:
    ...
    def __init__(self):
        ...
        self.root.title("View Pass")
        self.root.config(bg="royal blue")

        Label(self.root, text="View Monthly Passes ", font="rockwell 38 bold underline",fg="red",bg="orange",relief=SUNKEN,bd=6).pack(pady=10,ipadx=10)
        ...
        #self.obj.heading("id",anchor="center")
        self.obj.column("#1",width=100)
        #self.obj.column("#2",width=150)
        self.obj.column("#3",width=150)
        self.obj.column("#4",width=300)
    ...
    def viewPriceSearch(self):
        ...
        if self.f0txt1.get()==self.search_type[0]:
            q=f"Select * from monthly_pass where id={self.f0txt2.get()}"

        elif self.f0txt1.get()==self.search_type[1]:
            q = f"Select * from monthly_pass where vehicletype like ('%{self.f0txt2.get()}%')"

        elif self.f0txt1.get()==self.search_type[2]:
            q = f"Select * from monthly_pass where description like ('%{self.f0txt2.get()}%')"
    ...
    def onDoubleClick(self, event):
        ...
        q = f"select vehicletype from monthly_pass where id='{self.items[0]}'"
        cr.execute(q)
        result = cr.fetchone()

        self.r1 = Toplevel()
        self.r1.title('Update Pass')
        self.r1.geometry("600x550+0+0")
        self.r1.config(bg="green3")

        Label(self.r1, text="Update Monthly Pass", font="rockwell 30 bold",fg="chocolate2",bg="green").pack(fill=X,pady=20)
        ...
        Label(self.r1, text="", font=self.font2).pack()
    ...
    def updatePrice(self):
        ...
        if vtype == "" or pr == "" or des == "":
            ...
            q = f"update monthly_pass set vehicletype='{vtype}',price={pr},description='{des}' where id={vid}"
            cur.execute(q)
            conn.commit()
            self.getValues()
            showinfo("Success","Monthly Pass Updated Successfully!",parent=self.r1)
            self.r1.destroy()
    ...
    def deletePrice(self):
        ...
        if ans:
            conn = connection.Connect()
            cur = conn.cursor()
            q = f"delete from monthly_pass where id ={self.id}"
            cur.execute(q)
            conn.commit()
            self.getValues()
            showinfo("","Monthly Pass Deleted Successfully!",parent=self.r1)
            self.r1.destroy()
