...
class main:
    ...
    def __init__(self):
        ...
        self.root.title("Add Pass")
        self.root.config(bg="orange2")

        Label(self.root, text="Adding the Monthly Pass", font="rockwell 30 bold underline",fg="green",bg="seagreen2").pack(fill=X,pady=30)
        ...
        self.txt2.grid(row=1, column=1, pady=15)
        ...
        self.txt3.insert(0, 200)
        self.txt3.config(state="readonly")
        self.txt3.grid(row=2, column=1, pady=15)
    ...
    def add_(self, event=None):
        ...
        if vtype == "" or pr == "" or des== "" :
            ...
            q = f"Insert into monthly_pass values (NULL,'{vtype}',{pr},'{des}')"
            print(q)
            cr.execute(q)
            conn.commit()
            showinfo("Success", "Monthly Pass added successfully",)
            self.root.destroy()
    ...
    def autoPriceSelection(self, event):
        ...
        if vt == self.vtypes[0]:
            ...
            self.txt3.insert(0, 300)
        elif vt == self.vtypes[2]:
            self.txt3.insert(0, 500)
        elif vt == self.vtypes[3]:
            self.txt3.insert(0, 1000)
        else:
